...
class HailoDetector:
    def __init__(self, hef_path):
        if not HAS_HAILO:
            raise RuntimeError(
                "hailo_platform is not installed/importable. "
                "Run this on the Raspberry Pi with HailoRT installed."
            )

        hef_path = os.path.abspath(hef_path)
        if not os.path.exists(hef_path):
            raise FileNotFoundError(f"HEF detector not found: {hef_path}")
        ...
        self.output_names = [o.name for o in output_infos]

        self.in_h = int(self.input_shape[0])
        self.in_w = int(self.input_shape[1])

        self.input_params = hpf.InputVStreamParams.make(
            self.network_group,
        ...
        print(f"HEF input: {self.input_name} {self.input_shape}")
        print("HEF outputs:")
    ...
    def infer(self, frame):
        ...
        with self.network_group.activate(self.network_group_params):
            with hpf.InferVStreams(
                ...
                outputs = infer_pipeline.infer({self.input_name: inp})

        return parse_hailo_yolo_outputs(outputs, orig_w, orig_h, self.in_w, self.in_h)
...
def parse_hailo_yolo_outputs(outputs, orig_w, orig_h, in_w, in_h):
    tensors = {}

    print("HEF output tensors:")
    ...
    score_maps = sorted(score_maps, key=lambda x: x.shape[0], reverse=True)

    print(f"box maps: {[x.shape for x in box_maps]}")
    print(f"score maps: {[x.shape for x in score_maps]}")

    if len(box_maps) == 0 or len(score_maps) == 0:
        print("[WARN] Could not find matching box/score outputs.")
        return []

    all_boxes = []
    all_scores = []

    for box_map in box_maps:
        h, w, _ = box_map.shape

        matching_scores = None
        for sm in score_maps:
            if sm.shape[0] == h and sm.shape[1] == w:
                matching_scores = sm
                break

        if matching_scores is None:
            print(f"[WARN] No score map matched box map {box_map.shape}")
            continue

        boxes = box_map.reshape(-1, 4).astype(np.float32)
        scores = matching_scores.reshape(-1).astype(np.float32)
    ...
    keep_nms = nms_xyxy(all_boxes, all_scores, iou_thresh=0.45)
    ...
    for i in keep_nms:
        ...
        x2 = int(np.clip(x2 * sx, 0, orig_w - 1))
    ...
    if detections:
        print(f"best detection: {max(detections, key=lambda d: d[4])}")
...
def run_detector(model, frame):
    if isinstance(model, HailoDetector):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Pedestrian Signal Inference v4 - Hailo HEF detector version")
    ...
    if args.os == "mac":
        ...
        detector_path = os.path.abspath(args.detector or DETECTOR_HEF)
